...
- Aurora is a Symfony 8.1 bundle requiring PHP 8.4 or newer.
...
2. Create a new empty directory (e.g., /workspace/Aurora-installed) then run the following commands:
   ...
   2.3 `yes | composer create-project symfony/skeleton:8.1.x-dev . --no-cache`
   ...
   2.6 `yes | composer require sindla/aurora:8.1.x-dev -W --no-progress`
...
```
composer create-project symfony/skeleton:8.1 temp_project --no-cache
