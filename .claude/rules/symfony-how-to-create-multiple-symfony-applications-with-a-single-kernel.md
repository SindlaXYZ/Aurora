...
**Sources:**
* https://symfony.com/doc/8.1/configuration/multiple_kernels.html

This rule reproduces the Symfony 8.1 documentation page on running multiple applications from a single `Kernel`. The page, the `Kernel` class shape, and the `MicroKernelTrait` usage it shows are unchanged on Symfony 8.1 (verified against the 8.1 docs and `CHANGELOG-8.1.md`, which has no entries touching `MicroKernelTrait` multi-application behavior, `getConfigDir`, `getCacheDir`, `getLogDir`, `registerBundles`, or `APP_ID`).

> **Related option in 8.1 - HTTP-less kernels.** The pattern below uses the HTTP-full `MicroKernelTrait` + `Symfony\Component\HttpKernel\Kernel`, which is correct for applications that serve HTTP (the Dockraft stub does). When a SEPARATE application never serves HTTP (a CLI tool, a Messenger worker, a scheduler daemon), Symfony 8.1 also offers `AbstractKernel` + `KernelTrait` in the DependencyInjection component - a lighter, HTTP-less kernel base. See `.claude/rules/http-less-applications.md`. The two are independent: the multi-app `APP_ID` mechanism here can be combined with either kernel base; pick the HTTP-less one only for an app that genuinely does not serve requests.
...
configurations based on various environments, it can be adapted to run different
applications with specific bundles and configuration.
...
This example follows the [Shared Kernel](http://ddd.fed.wiki.org/view/shared-kernel) pattern: all applications maintain an
...
directory structure like the one described in the Symfony Best Practice:
...
the `Symfony\Bundle\FrameworkBundle\Test\WebTestCase` class by
...
> **Dockraft stub note.** The stub enforces a `#[Group(...)]` attribute on every test method (see `.claude/rules/phpunit.md`). If you split tests into a per-app suite as above, the same group-enforcement rule applies to the new `apps/<id>/tests/` directory.
