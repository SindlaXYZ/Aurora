...
class ComposerUpdateCommand extends ContainerAwareCommand
{
    ...
    {
        $this
            ->setName('aurora:composer-update')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->rootDir   = $this->container->getParameter('aurora')['root'];
        ...
        $IOService->recursiveCreateDirectory($this->rootDir . '/web/static/aurora/js/');

        copy(realpath(dirname(__FILE__)) . '/../Static/js/f.adblock.js', $this->rootDir . '/web/static/aurora/js/f.adblock.js');
        $this->output->writeln(sprintf('%s ... done;', $this->p()));
    ...
    {
        $phpUnitFile = $this->rootDir . '/phpunit.phar';
        ...
        if (!file_put_contents($phpUnitFile, $phar)) {
    ...
    public function updateGeoIP2Contry()
    {
        ...
        if (!is_dir($maxmindDir) && !mkdir($maxmindDir, 0777, true)) {
    ...
    public function clearTmpDir()
    {
        ...
        foreach (glob($this->container->getParameter('aurora')['tmp'] . '/', GLOB_ONLYDIR) as $directory) {
            $IOService->recursiveDelete($directory, true);
        }
    }
}